...
test_X = np.c_[test_X, test_ones]
pre_Y = func(W, test_X)
print('均方误差为:')

MSE = np.sum((pre_Y.reshape(-1,1) - test_Y)**2, axis = 0) / pre_Y.shape[0]
print(MSE)
limit_x = np.array([[min(test_X[...,0]),1],[max(test_X[...,1]),1]])
...
ax.plot(np.linspace(0,1,1000), func(W, make_x((np.linspace(0,1,1000).reshape(-1,1)))) , color = 'red', marker = '.', linestyle = '-', markersize = 1,label = '训练模型')
plt.legend()
ax.set_title('问题1：一维线性模型', fontsize = 20)
ax.set_xlabel('输入X')
ax.set_ylabel('输出Y')
plt.savefig('./第一问画图结果.png')
plt.show()
#print(data_test)
#print(data_train)